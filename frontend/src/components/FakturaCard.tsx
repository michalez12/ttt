...
import { PAYMENT_ICONS, PAYMENT_COLORS } from '../services/types';
...
export default function FakturaCard({ faktura, onToggleSelect, selected, onVerify }: FakturaCardProps) {
    ...
    <div className={getColorClass()}>
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          <div className="text-2xl">{PAYMENT_ICONS[faktura.forma_platnosci] || '❓'}</div>
          <div>
            <h3 className="font-semibold text-lg">{faktura.numer_faktury}</h3>
            <p className="text-sm text-gray-600">{faktura.kontrahent.nazwa}</p>
          </div>
        </div>
        <div className="flex items-center space-x-2">
          {faktura.rachunek && faktura.rachunek.status_biala_lista === 'ZWERYFIKOWANY' && (
            <CheckCircle className="w-5 h-5 text-green-500" />
          )}
          {faktura.rachunek && faktura.rachunek.status_biala_lista === 'NIEZWERYFIKOWANY' && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4 text-sm">
        <div>
          ...
          <div className="font-semibold">{faktura.kwota_brutto.toLocaleString('pl-PL')} {faktura.waluta}</div>
        </div>
        <div>
      ...
      <div className="flex space-x-2">
        {faktura.forma_platnosci === 6 && faktura.rachunek && (
          <button
            ...
            disabled={verifying || faktura.rachunek.status_biala_lista === 'ZWERYFIKOWANY'}
