...
export default function FakturyList() {
  ...
  const [sortDirection, setSortDirection] = useState<"none" | "asc" | "desc">("desc");
  ...
  const handleMarkPaid = async (id: number) => {
    try {
      ...
      setFaktury(prev => prev.map(f => f.id === id ? { ...f, status: "ZAPLACONA" } : f));
    } catch {
  ...
  const handleRevert = async (id: number) => {
    if (!window.confirm("Czy na pewno chcesz cofnąć status tej faktury do NOWA?")) return;
    try {
      await ksefApi.updateFakturaStatus(id, "NOWA");
      toast.success("Status zmieniony na NOWA");
      setFaktury(prev => prev.map(f => f.id === id ? { ...f, status: "NOWA" } : f));
    } catch {
  ...
  const handleRozlicz = async (id: number, rozliczona: boolean) => {
    try {
      ...
      toast.success(rozliczona ? "Korekta rozliczona" : "Cofnięto rozliczenie");
      setFaktury(prev => prev.map(f => f.id === id ? { ...f, czy_rozliczona: rozliczona } : f));
    } catch {
  ...
  const handleExport = async () => {
    if (selected.length === 0) { toast.error("Wybierz co najmniej jedną fakturę"); return; }
    const wybraneFaktury = faktury.filter(f => selected.includes(f.id));
    const nieDoEksportu = wybraneFaktury.filter(f => String(f.forma_platnosci) !== "6");
    if (nieDoEksportu.length > 0) {
      toast.error(`Te faktury nie mogą być wyeksportowane (forma płatności ≠ przelew): ${nieDoEksportu.map(f => f.numer_faktury).join(", ")}`);
      return;
    }
    setExporting(true);
    try {
  ...
  const toggleSelectAll = () => {
    const eksportowalne = sortedFaktury.filter(f => !onlyTransfer || String(f.forma_platnosci) === "6");
    if (selected.length === eksportowalne.length) setSelected([]);
    else setSelected(eksportowalne.map(f => f.id));
  };

  const getTerminColor = (termin: string | null, status: string, forma_platnosci: any) => {
    ...
    const diffDays = Math.ceil((terminDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${styles[status] || "bg-gray-100 text-gray-800 border border-gray-200"}`}>
        {status}
      </span>
    );
  };
  ...
  const getFormaPlatnosci = (kod: any) => {
    const formy: Record<string, { label: string; color: string; emoji: string }> = {
    ...
    const forma = formy[String(kod)] || { label: `Kod ${kod}`, color: "bg-gray-100 text-gray-800", emoji: "❔" };
    return (
      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${forma.color}`}>
  ...
  const filteredFaktury = faktury.filter(f => {
    ...
    if (filterSearch) {
      ...
      if (!(f.numer_faktury?.toLowerCase().includes(s) || f.kontrahent?.nazwa?.toLowerCase().includes(s))) return false;
    }
    if (sortYear && f.data_wystawienia && new Date(f.data_wystawienia).getFullYear().toString() !== sortYear) return false;
    if (sortMonth && f.data_wystawienia && (new Date(f.data_wystawienia).getMonth() + 1).toString() !== sortMonth) return false;
    return true;
  });
  ...
  const visibleEksportowalne = sortedFaktury.filter(f => !onlyTransfer || String(f.forma_platnosci) === "6");
  ...
  if (loading) return <div className="bg-white rounded-lg shadow-sm border p-6">Ładowanie faktur...</div>;
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg shadow-sm border p-4">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <select value={sortYear} onChange={e => setSortYear(e.target.value)} className="border rounded px-3 py-1.5 text-sm">
            <option value="">Wszystkie lata</option>
            {availableYears.map(y => <option key={y} value={y.toString()}>{y}</option>)}
          </select>
          <select value={sortMonth} onChange={e => setSortMonth(e.target.value)} className="border rounded px-3 py-1.5 text-sm">
          ...
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            <input type="checkbox" checked={onlyTransfer} onChange={e => setOnlyTransfer(e.target.checked)} className="rounded" />
            Tylko przelew
          </label>
          <div className="ml-auto flex gap-2">
            <button onClick={loadFaktury} className="px-3 py-1.5 text-sm rounded border hover:bg-gray-50">
              🔄 Odśwież
            </button>
            <button
            ...
            >
              {exporting ? "Eksportuję..." : `📤 Eksportuj (${selected.length})`}
            </button>
          </div>
        </div>
          ...
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-3">
                <input
                  type="checkbox"
                  checked={visibleEksportowalne.length > 0 && selected.length === visibleEksportowalne.length}
                  onChange={toggleSelectAll}
                  className="rounded text-blue-600"
                />
              </th>
              <th className="px-4 py-3 text-left font-medium text-gray-600">Numer</th>
              <th className="px-4 py-3 text-left font-medium text-gray-600">Kontrahent</th>
              <th className="px-4 py-3 text-left font-medium text-gray-600">
                <button
                  onClick={() => setSortDirection(prev => prev === "desc" ? "asc" : prev === "asc" ? "none" : "desc")}
              ...
              <th className="px-4 py-3 text-left font-medium text-gray-600">Termin płatności</th>
              <th className="px-4 py-3 text-right font-medium text-gray-600">Kwota brutto</th>
              <th className="px-4 py-3 text-left font-medium text-gray-600">Forma płatności</th>
              <th className="px-4 py-3 text-left font-medium text-gray-600">Status</th>
              <th className="px-4 py-3 text-left font-medium text-gray-600">Akcje</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {sortedFaktury.map(faktura => {
              const disabledRow = onlyTransfer && String(faktura.forma_platnosci) !== "6";
              const isKorekta = !!faktura.czy_korekta;

              return (
                <tr
                  key={faktura.id}
                  className={"hover:bg-gray-50 " + (disabledRow ? "opacity-60" : "") + (isKorekta ? " bg-amber-50" : "")}
                >
                  ...
                  <td className="px-4 py-3">{faktura.kontrahent?.nazwa || "Brak"}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    {faktura.data_wystawienia
                      ? format(new Date(faktura.data_wystawienia), "dd.MM.yyyy", { locale: pl })
                      : "-"}
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={getTerminColor(faktura.termin_platnosci, faktura.status, faktura.forma_platnosci)}>
                      {faktura.termin_platnosci
                        ? format(new Date(faktura.termin_platnosci), "dd.MM.yyyy", { locale: pl })
                        : "-"}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">{getFormaPlatnosci(faktura.forma_platnosci)}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    {isKorekta ? (
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold border ${faktura.czy_rozliczona ? "bg-green-100 text-green-800 border-green-200" : "bg-red-100 text-red-800 border-red-200"}`}>
  ...
  {disabledRow && (
    <span className="text-[10px] text-gray-400">Nieprzelew</span>
  )}
</td>
                </tr>
              );
